...
`;
    ...
    setupLabel(){
      this.labelpos=this.getAttr("labelpos", "center 0px");
      ...
      if(this.listeningToThisMidiController(channel, controlNumber)) {
        if(this.tagName=="WEBAUDIO-SWITCH"){
          ...
          case "toggle":
            ...
            break;
          case "kick":
            // @@MB: 144 = note on, 128 = note off. if noteOn using a keyboard and not a pad
            // this will trigger. Let's try with other midi controllers....
            if(event.data[0] === 144) this.setValue(1);
            // NoteOff
            else if(event.data[0] === 128) this.setValue(0);
            //  noteOn/noteoff with velocity, typically using a sensitive pad
  ...
  customElements.define("webaudio-switch", class WebAudioSwitch extends WebAudioControlsWidget {
    ...
    connectedCallback(){
      ...
      if(this.attachShadow)
        ...
        root=this;
        //@MB
        console.log("draggable : " + this.getAttribute("draggable"));
...
//      this.listOfWidgets = document.querySelectorAll("webaudio-knob,webaudio-slider,webaudio-switch");
    ...
    contextMenuLearn(){
      ...
      let menu=document.getElementById("webaudioctrl-context-menu");
      console.log(menu.knob.midiController);
      // remove from local storage the mapping corresponding to element with the value
      // of the cc number
      let values = JSON.parse(localStorage.getItem("WebAudioControlsMidiLearn"));
      values.forEach(elem => {
        if(elem.cc.cc) {
          if((elem.cc.cc === menu.knob.midiController.cc) && (elem.cc.channel === menu.knob.midiController.channel))
            elem.cc = {};
        }
      });
      localStorage.setItem("WebAudioControlsMidiLearn",JSON.stringify(values));


      
      menu.knob.midiController={};
      //localStorage.setItem("WebAudioControlsMidiLearn","");
      this.contextMenuClose();
    }
    preserveMidiLearn(){
      if(!opt.preserveMidiLearn)
